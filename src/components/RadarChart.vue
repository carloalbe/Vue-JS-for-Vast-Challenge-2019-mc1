<template>
  <div style="padding-top:30px">
    ...
    <b-row style="padding-left:100px;padding-right: 50px">
      ...
      <b-col>
        <b-form-checkbox v-model="show_avg" button-variant="danger" name="avgOnOff" switch>
          Show average
        </b-form-checkbox>
      </b-col>
      <b-col>
      ...
      <b-popover triggers="click" target="radar_info" title="Plot a district">
        Drag a district from the map and drop it inside this plot to visualize its values. You can also add it from the
        districts list.
      </b-popover>
...
export default {
  ...
  computed: {
    aggr_measures: function () {
      return this.theta.map((row) => {
        return this.aggr + ' ' + row
      })
    },
  },
  watch: {
  ...
  methods: {
    ...
    drawTrace(stateid) {
      let state = this.states.filter(function (s) {
        return s.id == stateid
      })[0];
