    ...
    <b-row>
      ...
      <b-row>
        <b-form-checkbox v-model="nuclear.show" name="nuclearOnOff" switch>
          Show Nuclear <img src="../assets/nuclear.svg" height="10px">
        </b-form-checkbox>
      </b-row>
      <b-row>
        <b-form-checkbox v-model="hospitals.show" name="hospitalsOnOff" switch>
          Show hospitals <img src="../assets/hospital.svg" height="10px">
        </b-form-checkbox>
    </b-row>
    </b-col>
      <b-col>
...
<b-row><div id="mapDiv" >
  <svg  @click="defaultZone"  id="mapSvg" width="400" height="350">
    <g  id="gmap" transform="scale(1.5) translate(0 0)"></g>
    <use xlink:href="../assets/nuclear.svg#nuclear_g" v-show="nuclear.show"  xmlns="http://www.w3.org/2000/svg"  transform="translate(250 150) scale(0.02)"></use>
    <g transform=" scale(0.025) translate(-200 0)" >
      <use xlink:href="../assets/hospital.svg#hospital_g"  :x="hospital.cx*60" :y="hospital.cy*60" v-for="hospital in hospitals.coords" :key="hospital.id" v-show="hospitals.show" ></use>
    </g>
  </svg>
  ...
  methods: {
    ...
    drawMap(){
      d3.select('#gmap').on('click',this.defaultZone).selectAll('path').data(this.states.slice(1)).enter().append('path')
        ...
        .on('touchstart',function(event){event.preventDefault()})
